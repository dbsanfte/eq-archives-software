...
import { comparisonUrl, readerUrl, recordId, ReaderSearchContext, searchReturnLinkHandlers } from '../reader/reader-utils';
...
export default function CaptureHistory({ identity, compareWith, returnTo = '' }) {
        ...
        <ol className="archive-capture-list">
          {records.map(result => <li key={result._meta.id}>
            ...
            <div className="archive-capture-actions">
              <a href={readerUrl(recordId(result), { find, returnTo })} {...searchReturnLinkHandlers} aria-label={`Read capture from ${captureDate(result)}`}>Read</a>
              {recordId(compareWith) && recordId(result) !== recordId(compareWith) && <a href={comparisonUrl(compareWith, result, { returnTo })} {...searchReturnLinkHandlers} aria-label={`Compare capture from ${captureDate(result)} with selected capture`}>Compare</a>}
